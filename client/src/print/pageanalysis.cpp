...
PageAnalysis::PageAnalysis(int wigth, int height, int coefficient, int semester, QWidget *parent) :
    PrintPage(wigth, height, coefficient, parent, semester)
{
}
...
void PageAnalysis::paintData(QPainter &painter)
{
    ...
    painter.drawText(m_half, Qt::AlignCenter, QString("%1-е полугодие\n"
                                                  ...
                                                  .arg(m_semester)
                     .arg(m_semester == 1 ? "1-го полугодия" : "2-го полугодия и за год в целом"));
    ...
    painter.drawText(m_cheefTitle, Qt::AlignCenter, QString("Заключение заведующего кафедрой о выполнении нагрузки и качестве работы\n"
                                                            " преподавателя в %1:")
                                                        .arg(m_semester == 1 ? "1-ом полугодии" : "2-ом полугодии и за год в целом"));
...
void PageAnalysis::setRects()
{
    ...
    m_teacherSign = QRect(m_leftBord, m_half.bottom() + (singleRow() * ROW_COF) * ROW_COUNT + point(7), m_rigthBord, singleRow() * 2);
    m_cheefTitle = QRect(m_leftBord, m_teacherSign.bottom() + point(3), m_rigthBord, singleRow() * 2);
    m_cheefSign = QRect(m_leftBord, m_cheefTitle.bottom() + (singleRow() * ROW_COF) * ROW_COUNT + point(5), m_rigthBord, singleRow() * 2);
